...
// MARK: - CRC-16/MODBUS Lookup Table

/// Precomputed CRC-16/MODBUS lookup table (256 entries).
///
/// Generated with polynomial 0xA001 (reversed 0x8005).
/// Each entry represents the CRC contribution for a single byte value 0x00-0xFF.
///
/// Using `InlineArray` for inline storage without heap allocation.
///
/// Reference: Modbus Serial Line Protocol and Implementation Guide V1.02
@usableFromInline
let crc16Table: InlineArray<256, UInt16> = [
...
/// Calculates CRC-16/MODBUS checksum using table lookup.
///
...
public func calculateModbusCRC16(_ bytes: Span<UInt8>) -> UInt16 {
    ...
    for i in bytes.indices {
        // index is always 0-255 due to & 0xFF mask, matching table size
        let index = Int((crc ^ UInt16(bytes[i])) & 0xFF)
...
public func verifyModbusCRC(_ frame: Span<UInt8>) -> Bool {
    ...
    // Calculate CRC over data portion using table lookup
    var crc: UInt16 = 0xFFFF
